...
def decodeMorse(morseCode):
    ...
    return morseCode.replace('.', list(MORSE_CODE.keys())[list(MORSE_CODE.values()).index('.')]).replace('-', list(MORSE_CODE.keys())[list(MORSE_CODE.values()).index('-')]).replace(' ', '')
...
decodedBits = decodedBits.split("    ")
print(decodedBits)
first = decodedBits[0]
first = first.split("  ")
second = decodedBits[1]
second = second.split("  ")
print(first)
print(second)
